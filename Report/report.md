...
通过运行 `Compete` 将编写的 AI 和若干个 Testcase 进行了对打。评测结果都通过 `make compete opponent=xxx` 运行得到，输出写到 `Tests` 目录下。文件命名 `test-A-100` 代表编写的 AI 作为 A 方与 100 号 Testcase 对战的结果。

由于测例比较多，大概得到了如下的很不精确的胜率数据：
