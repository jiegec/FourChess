...
v11 版本在 Saiblo 排行榜上最高观察到第 64 名，1255 分。
...
进一步阅读论文，[发现可以用图来代替树，从而复用结点](https://github.com/lightvector/KataGo/blob/master/docs/GraphSearch.md)，因此对代码进行了大量修改，得到一个基于有向无环图的实现。称这个版本为 v14。但这个实现的内存回收比较难做，不做回收会 OOM，因此先做了一个每 n 回合清理所有缓存的结点的逻辑；后来改成引用计数法，把很早搜出来的一些局面丢弃掉。称这个版本为 v15。
