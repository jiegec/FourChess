...
但这个实现的内存回收比较难做，不做回收会 OOM，因此先做了一个每 n 回合清理所有缓存的结点的逻辑，但是这样缓存的效果比较差；后来改成引用计数法，把很早搜出来的一些局面丢弃掉。称这个版本为 v15。v15 虽然解决了内存回收的问题，但是复用的结点也变少了。在 v15 的基础上，去掉引用计数，改为扫描哈希表，把哈希表里面那些从当前局面不可达的结点释放掉（根据棋子数量以及是否存在某个位置有冲突），这样不会 OOM。称这个版本为 v16。
